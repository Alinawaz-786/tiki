...
//    Fee public feesstruct = Fee(10,2,2,1);
    ...
    enum userType { WhiteListedSell,NormalSell,WhiteListedBuys,NormalBuys }
    ...
    */
    ...
    mapping (address => bool) public isNotFirstTx;
    ...
    function userTypeFee(userType _type) internal returns(uint256 bnbFee,uint256 lpFee,uint256 distributionFee,uint256 walletFee){
        return (feeMapping[_type].bnbFee,
    ...
    function calculateFee(address account ,bool isWhitelisted,uint amount) internal returns(uint256 bnbAmount,uint256 lpAmount,uint256 distributionAmount,uint256 walletAmount) {
        if(isWhitelisted && isNotFirstTx[account]) {
        (uint256 bnbFee,uint256 lpFee,uint256 distributionFee,uint256 walletFee) = userTypeFee(userType.NormalBuys);
        return (
            lpFee.mul(amount).div(100),
            bnbFee.mul(amount).div(100),
            walletFee.mul(amount).div(100),
            distributionFee.mul(amount).div(100)
        );
        }
        if(isWhitelisted && isNotFirstTx[account]) {
        (uint256 bnbFee,uint256 lpFee,uint256 distributionFee,uint256 walletFee) = userTypeFee(userType.WhiteListedBuys);
        return (
            lpFee.mul(amount).div(100),
            bnbFee.mul(amount).div(100),
            walletFee.mul(amount).div(100),
        ...
        if(isWhitelisted && isNotFirstTx[account]) {
            (uint256 bnbFee,uint256 lpFee,uint256 distributionFee,uint256 walletFee) = userTypeFee(userType.WhiteListedSell);
            return (
                lpFee.mul(amount).div(100),
                bnbFee.mul(amount).div(100),
                walletFee.mul(amount).div(100),
        ...
        if(isWhitelisted && !isNotFirstTx[account]) {
            (uint256 bnbFee,uint256 lpFee,uint256 distributionFee,uint256 walletFee) = userTypeFee(userType.NormalSell);
            return (
                lpFee.mul(amount).div(100),
                bnbFee.mul(amount).div(100),
                walletFee.mul(amount).div(100),
    ...
    function processTax(uint256 bnbAmount,uint256 lpAmount,uint256 distributionAmount,uint256 walletAmount) internal returns(bool){
        swapTokensForEth(bnbAmount);
        // addLiquidity(lpAmount); // how much ether against tokens
        swapAndSendDividends(distributionAmount);
        super._transfer(msg.sender, mainWallet, walletAmount0);
    ...
    function _transfer(
        ...
        if(isFixedSaleBuy) { //true
            require(block.timestamp >= fixedSaleStartTimestamp, "TIKI: The fixed-sale has not started yet."); // true
        ...
        if( 
            ...
            uint256 bnbAmount,uint256 lpAmount,uint256 distributionAmount,uint256 walletAmount = calculateFee(from,fixedSaleEarlyParticipants[from],amount); // calculate fee for selling
            
        }

		uint256 contractTokenBalance = balanceOf(address(this)); // 0
        
        bool canSwap = contractTokenBalance >= swapTokensAtAmount; // false
    ...
    feeMapping[userType.WhiteListedBuys].bnbFee = 10;
    feeMapping[userType.WhiteListedBuys].lPoolFee = 1;
    feeMapping[userType.WhiteListedBuys].redistributionFee = 1;
